...
def get_execution_status_from_classifiers_executions(classifiers_executions: list[dict]) -> str:
    """
...
async def run_in_batch(pool: psycopg_pool.AsyncConnectionPool) -> dict:
    """
    ...
    classifiers_executions = await classifiers_service.run_all_classifiers_in_batch(pool, execution, classifiers)
    logger.info(f"Executed {len(classifiers_executions)} classifiers")
    
    execution_status = get_execution_status_from_classifiers_executions(classifiers_executions)
    execution_finished = await finish_execution(pool, execution['execution_id'], execution_status)
